...
const movingUsers = new Set(); // Track users being moved to prevent duplicate events
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  // Ignore events for users being moved by the bot
  if (movingUsers.has(member.id)) {
    return;
  }
  ...
  if (
    ...
    // Mark user as being moved to prevent duplicate events
    movingUsers.add(member.id);
    ...
    if (tempChannel) {
      try {
        ...
        // Start voice session for leveling system
    ...
    // Remove user from moving set after a short delay
    setTimeout(() => {
      movingUsers.delete(member.id);
    }, 1000);
    
    return; // Exit early to prevent processing other logic
  }
  ...
  // User joined a voice channel (not from Create Room, that's handled above)
  if (!oldState.channelId && newState.channelId && newState.channelId !== CONFIG.CREATE_ROOM_CHANNEL_ID) {
    levelingSystem.startVoiceSession(member.id);
  }
